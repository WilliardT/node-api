...
class CybersportScraper extends BaseScraper {
  ...
  async getNews() {
    ...
    try {
      ...
      try {
        await page.waitForSelector('article, .news-item, [class*="news"]', {
          timeout: 10000,
        });
      } catch (timeoutError) {
      ...
      for (const selector of newsSelectors) {
        ...
        if (elements.length > 0) {
          console.log(
            `Using selector: ${selector}, found ${elements.length} elements`
          );
            ...
            try {
              ...
              for (const titleSelector of titleSelectors) {
                ...
                if (titleEl.length > 0) {
                  ...
                  linkElement = titleEl.is('a')
                    ? titleEl
                    : titleEl.closest('a').length > 0
                    ? titleEl.closest('a')
              ...
              if (!title || !linkElement || linkElement.length === 0) {
                ...
                if (linkElement.length > 0) {
                  title =
                    linkElement.attr('title') || linkElement.text().trim();
                }
              }

              const relativeUrl =
                linkElement && linkElement.length > 0
      ...
      if (!foundArticles) {
        console.log(
          'No articles found with any selector, trying fallback approach...'
        );

        // Fallback: ищем все ссылки, которые могут быть новостями
        $('a[href*="/news/"], a[href*="/article/"], a[href^="/"]').each(
          (index, element) => {
      ...
      console.log(
        `After deduplication: ${uniqueArticles.length} articles from ${this.name}`
      );
